...
def generate_prompt(instruction: str) -> str:
    logger.info("Rozpoczęto generowanie prompta.")
    prompt = f"""
    ...
    """
    logger.info("Prompt wygenerowany.")
    return prompt

# Funkcja do komunikacji z OpenAI API
def ask_chatgpt_with_prompt(instruction: str) -> str:
    try:
        logger.info("Rozpoczęto proces komunikacji z OpenAI API.")
        prompt = generate_prompt(instruction)
        logger.info(f"Wysłany prompt: {prompt}")
        ...
        logger.info(f"Odpowiedź z OpenAI API: {response}")
...
async def log_requests(request: Request, call_next):
    logger.info("Otrzymano nowe żądanie.")
...
@app.get("/")
def read_root():
    logger.info("Obsługa żądania testowego GET /.")
...
def process_map_instruction(instr: Instruction, apikey: Optional[str] = Header(None)):
    logger.info("Rozpoczęto obsługę żądania POST /map.")
    logger.info(f"Otrzymana instrukcja: {instr.instruction}")

    if apikey != API_KEY:
        logger.warning("Nieprawidłowy klucz API!")
        raise HTTPException(status_code=403, detail="Forbidden")

    logger.info("Klucz API poprawny. Wywoływanie funkcji ask_chatgpt_with_prompt...")
    ...
    try:
        logger.info(f"Próba parsowania odpowiedzi: {description}")
        result = json.loads(description)
        logger.info(f"Parsowanie zakończone sukcesem. Zwracany wynik: {result}")
        return result
